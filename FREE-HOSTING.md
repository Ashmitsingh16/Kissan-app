...
4. Import this repository into Vercel Hobby with Root Directory `frontend`. Set `NEXT_PUBLIC_DEMO_MODE=false` and `NEXT_PUBLIC_API_URL` to the actual Render backend HTTPS URL plus `/api`, then build. Vercel Hobby is for personal, non-commercial use; do not use it for a commercial launch without checking eligibility. Do not add secret backend values to the frontend environment.
