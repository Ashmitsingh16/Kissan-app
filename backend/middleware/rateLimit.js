...
const authIpLimiter = createLimiter({ scope: 'Kissan-app:auth-ip', limit: 100 });
const authAccountLimiter = createLimiter({ scope: 'Kissan-app:auth-account', limit: 10, key: emailKey });
const resetAccountLimiter = createLimiter({ scope: 'Kissan-app:reset-account', limit: 3, key: emailKey });
const notificationLimiter = createLimiter({ scope: 'Kissan-app:notification-account', limit: 30, key: userKey });
const notificationIpLimiter = createLimiter({ scope: 'Kissan-app:notification-ip', limit: 100 });
const aiLimiter = createLimiter({ scope: 'Kissan-app:ai-account', limit: 10, key: userKey });
const contactIpLimiter = createLimiter({ scope: 'Kissan-app:contact-ip', limit: 5 });
...
module.exports = { aiLimiter, createLimiter, authIpLimiter, authAccountLimiter, resetAccountLimiter,
